...
cat > components/course/CourseCard.test.tsx <<'EOF'
...
if npm test -- components/course/CourseCard.test.tsx; then
  echo 'Expected RED before premium CourseCard composition.' >&2
  exit 1
else
  echo 'RED confirmed: distinct course identity is not implemented.'
fi

cat > components/course/CourseCard.tsx <<'EOF'
