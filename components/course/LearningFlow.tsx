...
interface LearningFlowProps { topic: TopicDefinition; onOpenLab: () => void; onQuizComplete: (result: QuizSubmission) => void; }
const stages = ['Learn', 'Visualize', 'Practice', 'CS Connection', 'Quiz'] as const;

export const LearningFlow: React.FC<LearningFlowProps> = ({ topic, onOpenLab, onQuizComplete }) => (
  <div className="space-y-5" data-testid="learning-flow">
    ...
    <section className="notebook-panel p-5" aria-labelledby="stage-quiz"><h2 id="stage-quiz" className="stage-title"><Trophy />{stages[4]}</h2><div className="mt-4"><QuizRunner courseId={topic.courseId} topicId={topic.id} quiz={topic.quiz} onComplete={onQuizComplete} /></div></section>
  </div>
);
